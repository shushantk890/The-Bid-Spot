...
export const addNewAuctionItem = catchAsyncErrors(async (req, res, next) => {
  ...
  const {
    title,
    description,        //AUCTION ITEM ADDED
    category,
    condition,
    startingBid,
    startTime,
  ...
  try {
    const cloudinaryResponse = await cloudinary.uploader.upload(      //IMAGES UPLOADED ON CLOUDINARY
      image.tempFilePath,
      {
...
export const getAllItems = catchAsyncErrors(async (req, res, next) => {
  ...
  res.status(200).json({                                          //FUNCTION CREATED
    success: true,
    items,
  });
});
...
export const republishItem = catchAsyncErrors(async (req, res, next) => {
 ...
 //IF HIGHEST BIDER NOT PAYING THEN DECREASE MONEY PSEND BY BIDDER AND AUCTION WON =-1
  ...
  data.bids = [];                //WE NEED TO EMPTY THE ARRAY BEFORE REPUBLISH THAT ITEM AGAIN
