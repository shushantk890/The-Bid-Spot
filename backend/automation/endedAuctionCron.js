...
export const endedAuctionCron = () => {
  cron.schedule("*/1 * * * *", async () => {//setting up timer to occur function ,we used this function because written in javascript can be crashed in terminal
    const now = new Date();
    ...
    for (const auction of endedAuctions) {
      try {
        ...
        if (highestBidder) {
          ...
          await auction.save();
          //finding highest bidder and updating
          const bidder = await User.findById(highestBidder.bidder.id);
          await User.findByIdAndUpdate(
            bidder._id,
            {
              $inc: {      //calculating total bid amount by user
